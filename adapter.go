...
func mapToArray(m map[string]*ldapSD) []ldapSD {
	arr := make([]ldapSD, 0, len(m))
	for _, v := range m {
		arr = append(arr, *v)
	}
	return arr
...
func (a *Adapter) runLdapSD(ctx context.Context) {
